import { SolanaAssetType } from "../src";

import * as data from "./fixtures/data";

function assertInterface(result: SolanaAssetType) {
...
describe("SolanaAssetType", () => {
  it.only("should parse string", async () => {
    const result = SolanaAssetType.parse(data.ASSET_TYPE_STRING);
    expect(result).toEqual(data.ASSET_TYPE_NESTED_PARAMS);
  });

  it("should format params", async () => {
    const result = SolanaAssetType.format(data.ASSET_TYPE_PARAMS);
    expect(result).toEqual(data.ASSET_TYPE_STRING);
  });

  it("should instantiate from params", async () => {
    const result = new SolanaAssetType(data.ASSET_TYPE_PARAMS);
    assertInterface(result);
  });

  it("should instantiate from string", async () => {
    const result = new SolanaAssetType(data.ASSET_TYPE_STRING);
    assertInterface(result);
  });

  it("should instantiate from nested params", async () => {
    const result = new SolanaAssetType(data.ASSET_TYPE_NESTED_PARAMS);
    assertInterface(result);
  });
  ...
  it("should support JSON.stringify", async () => {
    ...
    assertInterface(new SolanaAssetType(json));
  });

  it("should throw an error if invalid AssetType is provided", async () => {
    expect(() => {
      new SolanaAssetType(data.INVALID_ASSET_TYPE_PARAMS);
    }).toThrow();
    expect(() => {
      new SolanaAssetType(data.INVALID_ASSET_TYPE_NESTED_PARAMS);
    }).toThrow();
  });

  it("should fail on parse if invalid AssetType is provided", async () => {
    expect(() => {
      SolanaAssetType.parse(data.INVALID_ASSET_TYPE_STRING);
    }).toThrow();
  });
});

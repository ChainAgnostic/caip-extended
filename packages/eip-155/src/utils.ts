...
export function isValidEIP155ChainId(
  ...
  if (!isValidEIP155NamespaceAndReference(params[0], params[1])) {
    return false;
  }
...
export function isValidEIP155AccountId(
  ...
  if (!isValidEIP155NamespaceAndReference(namespace, reference)) {
    return false;
  }
...
export function isValidEIP155NamespaceAndReference(
  namespace: string,
  reference: string
): boolean {
  if (namespace !== "eip155") {
